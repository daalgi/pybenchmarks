...
def python_list_range(length):
    return list(range(length))

def numpy_arange(length):
...
for length in lengths:
    ...
    if length < 2e4:
        ...
        Test(python_list_range, length, repetitions=repetitions, verbose=False)
    Test(numpy_arange, length, repetitions=repetitions, verbose=False)
